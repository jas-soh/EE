...
static void LCD_GPIOInit(void){
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTN_DIR_R |= (0x10 | 0x20);          // make PN4, PN5 outputs
    GPIO_PORTN_AFSEL_R &= ~(0x10 | 0x20);       // disable alternate functions
    GPIO_PORTN_DEN_R |= (0x10 | 0x20);          // enable digital I/O on PN4, PN5

    // P0, P1, P3, P4
    SYSCTL_RCGCGPIO_R |= 0x2000;                // activate port P
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTP_DIR_R |= (0x3 | 0x18);           // make PP0-1, PP3-4 outputs
    GPIO_PORTP_AFSEL_R &= ~(0x3 | 0x18);        // disable alternate functions
    GPIO_PORTP_DEN_R |= (0x3 | 0x18);           // enable digital I/O on PP0-1, PP3-4

    // Q0, Q2, Q3
    SYSCTL_RCGCGPIO_R |= 0x4000;                // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTQ_DIR_R |= (0x1 | 0xC);            // make PQ0, PQ2-3 outputs
    GPIO_PORTQ_AFSEL_R &= ~(0x1 | 0xC);         // disable alternate functions
    GPIO_PORTQ_DEN_R |= (0x1 | 0xC);            // enable digital I/O on PQ0, PQ2-3
}
